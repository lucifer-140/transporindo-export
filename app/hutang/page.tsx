    ...
    return (
        <div className="max-w-7xl mx-auto">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-slate-900">Hutang List (Accounts Payable)</h1>
                <Link
                    href="/hutang/create"
            ...
            <div className="bg-white shadow overflow-hidden sm:rounded-lg">
                <table className="min-w-full divide-y divide-slate-200">
                    <thead className="bg-slate-50">
                        <tr>
                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">No. Hutang</th>
                            ...
                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Paid For (Doc)</th>
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-slate-200">
                        {hutangs.length === 0 ? (
                            ...
                            hutangs.map((hutang) => (
                                <tr key={hutang.id}>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">{hutang.noHutang}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{new Date(hutang.date).toLocaleDateString()}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                                        {hutang.paymentType}
                                        {hutang.paymentType !== 'Cash' && (
                                            <span className="block text-xs text-gray-400">
                                                {hutang.giroNo} - {hutang.giroDate ? new Date(hutang.giroDate).toLocaleDateString() : '-'}
                                            </span>
                                        )}
                                    </td>
