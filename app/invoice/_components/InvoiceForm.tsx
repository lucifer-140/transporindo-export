...
export default function InvoiceForm({ initialData }: InvoiceFormProps) {
    ...
    useEffect(() => {
        ...
        if (initialData) {
            setFormData({
                jobId: initialData.jobId.toString(),
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // Manually append items to FormData for server action
        items.forEach((item, index) => {
            form.append(`items[${index}].type`, item.type);
            form.append(`items[${index}].description`, item.description);
            form.append(`items[${index}].qty`, item.qty.toString());
            form.append(`items[${index}].price`, item.price.toString());
            form.append(`items[${index}].total`, item.total.toString());
            if (item.date) form.append(`items[${index}].date`, item.date);
            if (item.dpp) form.append(`items[${index}].dpp`, item.dpp.toString());
            if (item.ppn) form.append(`items[${index}].ppn`, item.ppn.toString());
        });

        if (isEditMode) {
            await updateInvoice(initialData.id, form);
        } else {
            // For create, we need to restructure how items are sent if we want to reuse the same server action logic
            // Or we can just use the same logic in createInvoice to parse items like updateInvoice does
            // Let's update createInvoice to handle items parsing similar to updateInvoice first?
            // Actually, the previous createInvoice used JSON.stringify for items.
            // Let's adapt this form to use JSON.stringify for items for now to match createInvoice, 
            // BUT updateInvoice uses manual parsing. 
            // To be consistent, let's make updateInvoice use JSON.stringify too or vice versa.
            // The updateInvoice I wrote uses manual parsing. The createInvoice uses JSON.stringify.
            // I should stick to one. Let's use the hidden input JSON string approach for both for simplicity in this component.

            // Wait, I can't easily change the server action signature from here.
            // Let's just use the hidden input for items, it's easier for the form.
            // I need to update updateInvoice to parse JSON string items too.

            // Actually, let's just stick to the hidden input method for now as it's already in the create form.
            // I will update updateInvoice to parse JSON items.
    ...
    return (
        <div className="max-w-7xl mx-auto pb-20">
            <h1 className="text-2xl font-bold text-slate-900 mb-6">{isEditMode ? 'Edit Invoice' : 'Create New Invoice'}</h1>

            <form action={isEditMode ? (formData) => updateInvoice(initialData.id, formData) : createInvoice} className="space-y-8">
                <div className="bg-white shadow px-4 py-5 sm:rounded-lg sm:p-6">
                    <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">

                        <div className="sm:col-span-2">
                            <label className="block text-sm font-medium text-slate-700">Invoice No</label>
                            <input type="text" name="invoiceNo" required value={formData.invoiceNo} onChange={handleInputChange} className="mt-1 block w-full shadow-sm sm:text-sm border-slate-300 rounded-md border p-2" />
                        </div>

                        <div className="sm:col-span-2">
                            <label className="block text-sm font-medium text-slate-700">Date</label>
                            <input type="date" name="date" required value={formData.date} onChange={handleInputChange} className="mt-1 block w-full shadow-sm sm:text-sm border-slate-300 rounded-md border p-2" />
                        </div>
