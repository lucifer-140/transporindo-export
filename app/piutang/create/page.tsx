...
import { createPiutang, searchInvoices } from '../actions';

export default function CreatePiutangPage() {
    ...
    useEffect(() => {
        async function fetchInvoices() {
            const invs = await searchInvoices(customerFilter);
            setInvoices(invs);
        }
        // Debounce search
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <h1 className="text-2xl font-bold text-slate-900 mb-6">Create New Piutang (Receipt)</h1>

            <form action={createPiutang} className="space-y-6 bg-white shadow px-4 py-5 sm:rounded-lg sm:p-6">
                <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">

                    {/* Payment Details */}
                    <div className="sm:col-span-3">
                        <label htmlFor="noPiutang" className="block text-sm font-medium text-slate-700">No. Piutang</label>
                        <input type="text" name="noPiutang" id="noPiutang" required className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" />
                    </div>
                            ...
                            <table className="min-w-full divide-y divide-slate-300">
                                <thead className="bg-slate-50">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-slate-900">Total Bon</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-200 bg-white">
                                    {invoices.map((inv) => (
                                        <tr key={inv.id} className={selectedInvoice?.id === inv.id ? 'bg-blue-50' : ''}>
                                            ...
                                            <td className="whitespace-nowrap px-3 py-4 text-sm text-slate-500">{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(calculateTotal(inv.items))}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
